...
void wait_for_replay_to_reach_end() {
    ...
    FAIL() << "Replay did not reach the end before the test deadline; progress " << currentEntry << " / "
           << totalEntries;
}
...
TEST_F(GeoQikTestApi, SaveLoadLogRoundTrip) {
    const std::filesystem::path firstPath =
        std::filesystem::temp_directory_path() / "geoqik_api_log_roundtrip_first.gqklog";
    const std::filesystem::path secondPath =
        std::filesystem::temp_directory_path() / "geoqik_api_log_roundtrip_second.gqklog";
...
TEST_F(GeoQikTestApi, SaveLoadLogRoundTripJson) {
    const std::filesystem::path firstPath =
        std::filesystem::temp_directory_path() / "geoqik_api_log_roundtrip_first.gqklog.json";
    const std::filesystem::path secondPath =
        std::filesystem::temp_directory_path() / "geoqik_api_log_roundtrip_second.gqklog.json";
    std::filesystem::remove(firstPath);
    std::filesystem::remove(secondPath);
...
TEST_F(GeoQikTestApi, SaveLoadLogValidation) {
    ...
    EXPECT_EQ(GEOQIK_ERROR_UNSUPPORTED_FORMAT,
              geoqik_save_log(validPath.string().c_str(), static_cast<geoqik_log_format_t>(42)));
    EXPECT_EQ(GEOQIK_ERROR_UNSUPPORTED_FORMAT,
              geoqik_load_log(validPath.string().c_str(), static_cast<geoqik_log_format_t>(42)));
    EXPECT_EQ(GEOQIK_ERROR_IO, geoqik_load_log(missingPath.string().c_str(), GEOQIK_LOG_FORMAT_BINARY));

    const std::filesystem::path validJsonPath =
        std::filesystem::temp_directory_path() / "geoqik_api_validation_log.gqklog.json";
    const std::filesystem::path missingJsonPath =
        std::filesystem::temp_directory_path() / "geoqik_api_missing_log.gqklog.json";
...
TEST_F(GeoQikTestApi, ReplayLogValidation) {
    const std::filesystem::path validPath =
        std::filesystem::temp_directory_path() / "geoqik_api_replay_validation_log.gqklog";
    const std::filesystem::path missingPath =
        std::filesystem::temp_directory_path() / "geoqik_api_replay_missing_log.gqklog";
    std::filesystem::remove(validPath);
    std::filesystem::remove(missingPath);
    ...
    EXPECT_EQ(GEOQIK_ERROR_UNSUPPORTED_FORMAT,
              geoqik_replay_log(validPath.string().c_str(), static_cast<geoqik_log_format_t>(42), nullptr));
    ...
    EXPECT_EQ(GEOQIK_ERROR_NOT_INITIALIZED,
              geoqik_replay_log(validPath.string().c_str(), GEOQIK_LOG_FORMAT_BINARY, &customKeyOptions));

    EXPECT_EQ(GEOQIK_ERROR_NOT_INITIALIZED,
              geoqik_replay_log(validPath.string().c_str(), GEOQIK_LOG_FORMAT_BINARY, nullptr));
...
TEST_F(GeoQikTestApi, ReplayLogRoundTrip) {
    const std::filesystem::path firstPath =
        std::filesystem::temp_directory_path() / "geoqik_api_replay_roundtrip_first.gqklog";
    const std::filesystem::path secondPath =
        std::filesystem::temp_directory_path() / "geoqik_api_replay_roundtrip_second.gqklog";
    std::filesystem::remove(firstPath);
    std::filesystem::remove(secondPath);
...
TEST_F(GeoQikTestApi, ReplayCurrentLogRoundTrip) {
    const std::filesystem::path beforePath =
        std::filesystem::temp_directory_path() / "geoqik_api_replay_current_before.gqklog";
    const std::filesystem::path afterPath =
        std::filesystem::temp_directory_path() / "geoqik_api_replay_current_after.gqklog";
...
TEST_F(GeoQikTestApi, ReplayDefersQueuedMessagesUntilManualEnd) {
    const std::filesystem::path replayPath =
        std::filesystem::temp_directory_path() / "geoqik_api_replay_deferred_source.gqklog";
    const std::filesystem::path actualPath =
        std::filesystem::temp_directory_path() / "geoqik_api_replay_deferred_actual.gqklog";
...
TEST_F(GeoQikTestApi, ReplayRemainsActiveAtEndForManualReverse) {
    const std::filesystem::path path =
        std::filesystem::temp_directory_path() / "geoqik_api_replay_manual_reverse.gqklog";
