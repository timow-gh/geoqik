...
TEST_F(GeometryBufferTest, AddPoints_Multiple) {
    ...
    std::vector<float> moreColors = {8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f, 18.0f, 19.0f};
...
TEST_F(GeometryBufferTest, AddLines_WithHandle) {
    ...
    std::vector<float> lines = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f};
    std::vector<float> colors = {0.1f, 0.2f, 0.3f, 0.4f, 0.5f, 0.6f, 0.7f, 0.8f};
...
TEST_F(GeometryBufferTest, UpdatePointsCoversSingleHandleAndSharedBulkColor) {
    ...
    EXPECT_THROW(
        { static_cast<void>(buffer->update_points(singleId, invalidPointCount, noColors)); },
        std::runtime_error);
    ...
    EXPECT_EQ(to_vector(buffer->get_points()),
              (std::vector<float>{10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f, 18.0f}));
...
TEST_F(GeometryBufferTest, PointBulkTransformsAffectOnlyMatchingHandle) {
    ...
    EXPECT_EQ(to_vector(buffer->get_points()),
              (std::vector<float>{2.0f, 2.0f, 3.0f, 1.0f, 3.0f, 3.0f, 10.0f, 20.0f, 30.0f}));
...
TEST_F(GeometryBufferTest, UpdateSingleAndBulkLines) {
    ...
    const std::vector<float>
        addedLines{7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f, 18.0f};
    ...
    const std::vector<float>
        bulkLines{26.0f, 27.0f, 28.0f, 29.0f, 30.0f, 31.0f, 32.0f, 33.0f, 34.0f, 35.0f, 36.0f, 37.0f};
    const std::vector<float> bulkColors{0.5f, 0.6f, 0.7f, 0.8f, 0.9f, 1.0f, 0.1f, 0.2f};
...
TEST_F(GeometryBufferTest, UpdateLinesCoversSingleHandleAndSharedBulkColor) {
    ...
    buffer->add_line(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, &singleId);
    const std::vector<float>
        originalBulk{7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f, 18.0f};
    const std::vector<float> noColors;
    ...
    EXPECT_THROW(
        { static_cast<void>(buffer->update_lines(singleId, invalidLineCount, noColors)); },
        std::runtime_error);

    const std::vector<float>
        wrongSingleCount{19.0f, 20.0f, 21.0f, 22.0f, 23.0f, 24.0f, 25.0f, 26.0f, 27.0f, 28.0f, 29.0f, 30.0f};
    ...
    const std::vector<float>
        updatedBulk{25.0f, 26.0f, 27.0f, 28.0f, 29.0f, 30.0f, 31.0f, 32.0f, 33.0f, 34.0f, 35.0f, 36.0f};
    const std::vector<float> sharedColor{0.5f, 0.6f, 0.7f, 0.8f};
    EXPECT_TRUE(buffer->update_lines(bulkId, updatedBulk, sharedColor));

    EXPECT_EQ(to_vector(buffer->get_lines()),
              (std::vector<float>{19.0f,
                                  20.0f,
                                  21.0f,
                                  22.0f,
                                  23.0f,
    ...
    EXPECT_EQ(to_vector(buffer->get_line_colors()),
              (std::vector<float>{0.1f, 0.2f, 0.3f, 0.4f, 0.1f, 0.2f, 0.3f, 0.4f, 0.5f, 0.6f, 0.7f, 0.8f,
                                  0.5f, 0.6f, 0.7f, 0.8f, 0.5f, 0.6f, 0.7f, 0.8f, 0.5f, 0.6f, 0.7f, 0.8f}));
}
...
TEST_F(GeometryBufferTest, BulkLineRemovalUpdatesFollowingMappings) {
    ...
    const std::vector<float> noColors;
    buffer->add_lines(std::vector<float>{1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f},
                      noColors,
                      &firstBulkId);
