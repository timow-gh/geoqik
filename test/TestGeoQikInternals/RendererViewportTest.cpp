...
TEST(RendererViewportTest, TranslatesWindowCoordinatesToSceneFramebufferCoordinates)
{
  ...
  const auto scenePoint =
      renderer::Renderer::to_scene_framebuffer_coordinates(viewport, 400.0, 100.0);
...
TEST(RendererViewportTest, RejectsSidebarAndOutsideWindowCoordinates)
{
  ...
  EXPECT_FALSE(renderer::Renderer::to_scene_framebuffer_coordinates(viewport, 327.0, 250.0)
                   .has_value());
  EXPECT_FALSE(renderer::Renderer::to_scene_framebuffer_coordinates(viewport, 1000.0, 250.0)
                   .has_value());
  EXPECT_FALSE(renderer::Renderer::to_scene_framebuffer_coordinates(viewport, 400.0, 500.0)
                   .has_value());
}
