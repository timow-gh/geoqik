...
TEST(SceneTest, CapacityGrowthUsesSettingsGrowthFactor) {
    ...
    EXPECT_EQ(scene.get_point_buffer().get_point_capacity(),
              settings.initialPointCapacity * settings.capacityGrowthFactor);
    EXPECT_EQ(scene.get_line_buffer().get_line_capacity(),
              settings.initialLineCapacity * settings.capacityGrowthFactor);
}
