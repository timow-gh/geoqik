...
[[nodiscard]] inline std::vector<std::uint8_t> make_vertex_translucency_flags(std::span<const float> colors, std::int32_t colorDimension)
{
...
[[nodiscard]] inline std::vector<linal::float3> make_vertex_sort_positions(std::span<const float> vertices, std::int32_t vertexDimension)
{
...
[[nodiscard]] inline linal::float3 get_sort_position_or_origin(std::span<const linal::float3> positions, std::uint32_t index) noexcept
{
...
[[nodiscard]] inline PointTransparencyIndexSplit split_point_indices_by_transparency(std::span<const std::uint32_t> indices,
                                                                                     ...
                                                                                     std::span<const std::uint8_t> vertexTranslucency)
{
  ...
  for (std::uint32_t index: indices)
  {
    ...
    {
      split.translucentIndices.push_back(SortablePointIndex{index, get_sort_position_or_origin(positions, index)});
    }
    else
    {
...
[[nodiscard]] inline LineTransparencyIndexSplit split_line_indices_by_transparency(std::span<const std::uint32_t> indices,
                                                                                   std::span<const linal::float3> positions,
                                                                                   std::span<const std::uint8_t> vertexTranslucency)
{
  ...
  for (std::size_t i = 0; i + 1 < indices.size(); i += 2)
  {
    ...
    if (is_vertex_translucent(vertexTranslucency, firstIndex) || is_vertex_translucent(vertexTranslucency, secondIndex))
    {
      split.translucentSegments.push_back(SortableLineSegment{firstIndex,
                                                              secondIndex,
...
[[nodiscard]] inline double distance_squared_to(const linal::float3& sortCenter, const linal::double3& viewPosition) noexcept
{
  const double dx = static_cast<double>(sortCenter[0]) - viewPosition[0];
  const double dy = static_cast<double>(sortCenter[1]) - viewPosition[1];
  const double dz = static_cast<double>(sortCenter[2]) - viewPosition[2];
                                                                                             ...
                                                                                             const linal::double3& viewPosition)
{
  ...
  std::sort(sortedPoints.begin(),
            ...
            { return distance_squared_to(lhs.sortCenter, viewPosition) > distance_squared_to(rhs.sortCenter, viewPosition); });
...
[[nodiscard]] inline std::vector<std::uint32_t> sort_translucent_line_indices_back_to_front(std::span<const SortableLineSegment> segments,
                                                                                            const linal::double3& viewPosition)
{
