...
public:
  ...
  Camera(std::uint32_t width, std::uint32_t height, const glm::dvec3& position, const glm::dvec3& target, const glm::dvec3& vertical)
  ...
  [[nodiscard]] glm::mat4 get_ortho_mvp() const { return get_ortho_projection_matrix() * get_view_matrix() * m_model; }
  ...
  [[nodiscard]] PickRay get_ray(double screenX, double screenY) const
  {
    switch (m_activeProjection)
    {
      case CameraProjectionType::PERSPECTIVE: 
        return create_perspective_ray(screenX, screenY);
   ...
   */
  [[nodiscard]] std::vector<PickRay> get_rays_for_region(
  ...
  {
    return screenX >= 0 && screenX < static_cast<double>(m_viewport.get_width()) &&
           screenY >= 0 && screenY < static_cast<double>(m_viewport.get_height());
  }
...
private:
  ...
  void update_projection_matrix() const
  {
    if (m_projectionDirty)
    {
      ...
      {
        case CameraProjectionType::PERSPECTIVE:
          m_projection = compute_perspective_matrix();
          break;
        case CameraProjectionType::ORTHOGRAPHIC:
