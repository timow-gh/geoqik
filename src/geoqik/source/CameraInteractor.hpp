...
public:
  ...
  void on_cursor_position(double xpos, double ypos)
  {
    ...
    if (m_prevXPos == std::numeric_limits<double>::quiet_NaN() || m_prevYPos == std::numeric_limits<double>::quiet_NaN())
    {
        m_prevXPos = xpos;
        m_prevYPos = ypos;
        return;
    ...
    if (m_cameraMode == CameraMode::ORBIT)
    {
      ...
      const glm::dquat zQuat = glm::angleAxis(glm::radians(deltaXPos * m_rotateSpeed), glm::dvec3{vertical[0], vertical[1], vertical[2]});
      const glm::dquat rightQuat = glm::angleAxis(glm::radians(-deltaYPos * m_rotateSpeed), glm::dvec3{right[0], right[1], right[2]});
