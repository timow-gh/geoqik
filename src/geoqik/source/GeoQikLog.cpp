...
std::optional<GeoQikLogEntry> create_log_entry(const GeoQikMessage& message) {
    return std::visit(
        [](const auto& value) -> std::optional<GeoQikLogEntry> {
            ...
            if constexpr (std::is_same_v<T, AddPointWithOpts> || std::is_same_v<T, AddPointsWithOpts> ||
                          std::is_same_v<T, UpdatePointWithOpts> || std::is_same_v<T, UpdatePointsWithOpts> ||
                          std::is_same_v<T, RemovePoint> || std::is_same_v<T, SetPointSize> ||
                          std::is_same_v<T, SetPointColor> || std::is_same_v<T, AddLineWithOpts> ||
                          std::is_same_v<T, AddLinesWithOpts> || std::is_same_v<T, UpdateLineWithOpts> ||
